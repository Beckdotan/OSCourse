...
void append_file_reverse(const char *source_file, const char *dest_file, int buffer_size, int max_size, int force_flag) {
	...
	/*
	 -------------- SOURCE FILE -------------
	*/
	//getting src file descriptor
	int src_des = open(source_file, O_WRONLY);
	if (src_des == -1){
		perror("Unable to open source file for reading");
		exit(EXIT_FAILURE);
	}

	//open stream to the surce file
	FILE *srcf = fdopen(src_des, "r");
	if (srcf == NULL){
	...
	int file_length = lseek(srcf, 0, SEEK_END)

	/*
	 -------------- DESTINAITION FILE -------------
	*/

	//open stearm for destination file. 
	int dst_des
	if (force_flag == 1){
		dst_des = open (dest_file, O_WRONLY | O_CREAT , 0777);		
	}else{
		dst_des = open (dest_file, O_WRONLY);
	}
	//if didnt work close srcf
	if (scr_des == -1){
		if(close(srcf) == -1){
	...
	*/
	...
	while (count < max_size && count < file_length){
		read_stream_reverse(srcf, buffer, buffer_size);
		c = write(dstf, buffer, buffer_size);
		if (c == -1){
			if(close(srcf) == -1){
				perror("couldn't close source file");
                exit(EXIT_FAILURE);
